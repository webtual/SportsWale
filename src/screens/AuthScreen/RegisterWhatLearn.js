...
import { View, Text, SafeAreaView, StyleSheet, TextInput, TouchableOpacity, ScrollView, Platform, FlatList,Image } from 'react-native'
...
import { ADD_GAME_LEARN_TO_PROFILE, ADD_GAME_TO_PROFILE, GET_GAMES, GET_GAME_LEARN_TO_PROFILE, IMAGE_URL, REMOVE_GAME_LEARN_TO_PROFILE, REMOVE_GAME_TO_PROFILE } from '../../constants/ApiUrl'
...
const RegisterWhatLearn = ({ route}) => {
    ...
    const Api_Add_game_to_profile = (data) => {
        ApiManager.post(ADD_GAME_LEARN_TO_PROFILE, {
            apikey: '123',
    ...
    const Api_Remove_game_to_profile = (data) => {
        ...
        ApiManager.post(REMOVE_GAME_LEARN_TO_PROFILE, {
            apikey: '123',
            users_games_learn_id: data?.users_games_learn_id,
        }).then((response) => {
            var data = response.data;
    ...
    const Api_Get_game_to_profile = (data) => {
        ApiManager.post(GET_GAME_LEARN_TO_PROFILE, {
            apikey: '123',
    ...
    const OnPressNext = () => {
        ...
        if(selectedList.length < 1){
            alert("Please select atleast one game")
        }
        else{
